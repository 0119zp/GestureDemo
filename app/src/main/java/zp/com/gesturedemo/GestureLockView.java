...
public class GestureLockView extends View {
    ...
    private boolean isGone;//绘制手势密码不显示线条轨迹标志
        ...
        if(cycles==null&&(getWidth()>0)&&(getHeight()>0)){
            ...
            for(int i=0;i<3;i++){
                for(int j=0;j<3;j++)
                {
                    ...
                    if (TextUtils.isEmpty(Constant.gestureStr)){
                        Constant.gestureStr="123456789";
                    }
                    lockCircle.setNum(String.valueOf(Constant.gestureStr.charAt(i*3+j)));//设置圈圈对应的数字
    ...
    public void setPsw(){
        for(int i=0;i<3;i++) {
            for(int j=0;j<3;j++) {
                ...
                cycles[i*3+j].setNum(String.valueOf(Constant.gestureStr.charAt(i*3+j)));//设置圈圈对应的数字
            }
        }
    }
